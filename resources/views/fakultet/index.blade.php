<x-app-layout>
    ...
    <div class="py-10 max-w-7xl mx-auto px-6">
        <div class="flex items-center justify-between mb-6">
            ...
            <button id="addFacultyBtn" class="bg-indigo-600 hover:bg-indigo-700 text-white font-semibold px-4 py-2 rounded-lg shadow-lg transform transition hover:scale-105 flex items-center">
        ...
        <div class="mb-4">
            <form action="{{ route('fakulteti.index') }}" method="GET" class="w-full max-w-md">
                <div class="relative">
                    ...
                    @if(request('search'))
                        <a href="{{ route('fakulteti.index') }}" class="absolute right-3 top-2.5 text-gray-400 hover:text-gray-600">
                ...
                <table class="min-w-full divide-y divide-gray-200">
                    <thead class="bg-gray-50">
                        <tr>
                            ...
                            <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider hidden sm:table-cell">Država</th>
                            <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider hidden lg:table-cell">Telefon</th>
                            <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider hidden xl:table-cell">Web</th>
                            <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider hidden lg:table-cell">Univerzitet</th>
                    ...
                    @foreach($fakulteti as $f)
                        <tr class="faculty-row hover:bg-gray-50 transition-colors duration-150 ease-in-out">
                            ...
                            <td class="px-6 py-4 text-sm text-gray-500 hidden xl:table-cell">
                                @if($f->web)
                                    <a href="{{ $f->web }}" target="_blank" class="text-blue-600 hover:underline">{{ Str::limit($f->web, 20) }}</a>
                                @endif
                            </td>
                            <td class="px-6 py-4 text-sm text-gray-500 hidden lg:table-cell">{{ $f->univerzitet->naziv }}</td>
