...
<div class="py-10 max-w-7xl mx-auto px-6">
    ...
    <div class="mb-4 flex flex-col items-start space-y-3">

        <div class="flex items-center space-x-2">
            <form id="bulkDeleteForm" action="{{ route('fakulteti.bulkDelete') }}" method="POST">
                @csrf
                @method('DELETE')
            ...
            <table class="min-w-full divide-y divide-gray-200">
                <thead class="bg-gray-50">
                    <tr>
                        ...
                        <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Univesity</th>
                    ...
                    @foreach($fakulteti as $f)
                        <tr class="faculty-row hover:bg-gray-50 transition-colors duration-150 ease-in-out" data-search="{{ strtolower($f->naziv . ' ' . $f->email . ' ' . $f->univerzitet->naziv) }}">
                            <td class="px-6 py-4 whitespace-nowrap">
                                <input type="checkbox" name="ids[]" value="{{ $f->id }}" class="selectCheckbox" form="bulkDeleteForm">
                            </td>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    deleteSelectedBtn.addEventListener('click', () => {
        if (confirm('Da li ste sigurni da želite obrisati selektovane fakultete?')) {
