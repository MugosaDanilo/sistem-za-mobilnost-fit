<x-app-layout>
    ...
    <div class="py-10 max-w-7xl mx-auto px-6">
        ...
        <div class="mb-4">
            <input type="text" id="searchFaculty" placeholder="Pretraži.."
        ...
        <div class="bg-white shadow-sm rounded-xl overflow-hidden border border-gray-200">
            <div class="px-6 py-4 border-b border-gray-200 bg-gray-50 flex justify-between items-center">
                ...
                <span
                    class="bg-blue-100 text-blue-800 text-xs font-medium px-2.5 py-0.5 rounded-full">{{ count($fakulteti) }}
                    Total</span>
            </div>
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        @foreach($fakulteti as $f)
                            <tr class="faculty-row hover:bg-gray-50 transition-colors duration-150 ease-in-out"
                                data-search="{{ strtolower($f->naziv . ' ' . $f->email . ' ' . $f->univerzitet->naziv) }}">
                                ...
                                <td class="px-6 py-4 text-sm text-gray-500">
                                    @if($f->web)
                                        <a href="{{ $f->web }}" target="_blank"
                                            class="text-blue-600 hover:underline">{{ $f->web }}</a>
                                    @endif
                                </td>
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-center text-sm font-medium">
                                    <div class="flex justify-center space-x-2">
                                        <button
                                            class="text-indigo-600 hover:text-indigo-900 bg-indigo-50 hover:bg-indigo-100 px-3 py-1 rounded-md transition-colors openEditModal"
                                            data-id="{{ $f->id }}" data-naziv="{{ $f->naziv }}" data-email="{{ $f->email }}"
                                            data-telefon="{{ $f->telefon }}" data-web="{{ $f->web }}"
                                            data-uputstvo="{{ $f->uputstvo_za_ocjene }}"
                                            data-univerzitet="{{ $f->univerzitet_id }}">
                                            Izmijeni
>>>>>>> Stashed changes
                                        ...
                                        <form action="{{ route('fakulteti.destroy', $f->id) }}" method="POST"
        ...
        <div class="bg-white rounded-lg shadow-lg w-full max-w-md p-6 relative overflow-y-auto max-h-screen">
            ...
            <form action="{{ route('fakulteti.store') }}" method="POST">
                ...
                <div class="mb-4">
                    ...
                    <input type="text" id="addPhone" name="telefon"
                        class="w-full border-gray-300 rounded-lg shadow-sm focus:border-blue-500 focus:ring-blue-500"
                        required>
                </div>
                ...
                <div class="mb-4">
                    ...
                    <textarea id="addInstructions" name="uputstvo_za_ocjene"
        ...
        <div class="bg-white rounded-lg shadow-lg w-full max-w-md p-6 relative overflow-y-auto max-h-screen">
            ...
            <form id="editFacultyForm" method="POST">
                ...
                <div class="mb-4">
                    ...
                    <input type="text" id="editPhone" name="telefon"
                        class="w-full border-gray-300 rounded-lg shadow-sm focus:border-blue-500 focus:ring-blue-500"
                        required>
                </div>
                ...
                <div class="mb-4">
                    <label for="editInstructions" class="block text-gray-700 font-medium mb-1">Uputstvo za
                        ocjene</label>
                    <textarea id="editInstructions" name="uputstvo_za_ocjene"
                        class="w-full border-gray-300 rounded-lg shadow-sm focus:border-blue-500 focus:ring-blue-500"></textarea>
                </div>
